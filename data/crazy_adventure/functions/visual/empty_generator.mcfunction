...
execute if score @s energy.storage < @s energy.max_storage run function crazy_adventure:work/empty_generator
...
#Update model depends on BurnTime (on/off)
	execute if data block ~ ~ ~ {BurnTime:0s} run data modify entity @s Item.tag.CustomModelData set value 4220300
	execute unless data block ~ ~ ~ {BurnTime:0s} run data modify entity @s Item.tag.CustomModelData set value 4220301
