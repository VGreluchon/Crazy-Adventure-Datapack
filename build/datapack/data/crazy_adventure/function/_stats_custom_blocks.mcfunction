
#> crazy_adventure:_stats_custom_blocks
#
# @within	???
#

scoreboard players add #total_custom_blocks crazy_adventure.data 0
scoreboard players add #total_vanilla_polished_deepslate crazy_adventure.data 0
scoreboard players add #total_vanilla_obsidian crazy_adventure.data 0
scoreboard players add #total_vanilla_jungle_sapling crazy_adventure.data 0
scoreboard players add #total_vanilla_iron_block crazy_adventure.data 0
scoreboard players add #total_vanilla_gold_block crazy_adventure.data 0
scoreboard players add #total_vanilla_furnace crazy_adventure.data 0
scoreboard players add #total_vanilla_end_rod crazy_adventure.data 0
scoreboard players add #total_vanilla_emerald_block crazy_adventure.data 0
scoreboard players add #total_vanilla_cobblestone crazy_adventure.data 0
scoreboard players add #total_vanilla_barrel crazy_adventure.data 0
scoreboard players add #total_banana_plant crazy_adventure.data 0
scoreboard players add #total_banana_sapling crazy_adventure.data 0
scoreboard players add #total_fertilizer_stick_charger crazy_adventure.data 0
scoreboard players add #total_shield_stick_charger crazy_adventure.data 0
scoreboard players add #total_fireball_stick_charger crazy_adventure.data 0
scoreboard players add #total_storm_stick_charger crazy_adventure.data 0
scoreboard players add #total_oxidizer crazy_adventure.data 0
scoreboard players add #total_uranium_upgrader crazy_adventure.data 0
scoreboard players add #total_ore_extractor_4 crazy_adventure.data 0
scoreboard players add #total_ore_extractor_3 crazy_adventure.data 0
scoreboard players add #total_ore_extractor_2 crazy_adventure.data 0
scoreboard players add #total_ore_extractor_1 crazy_adventure.data 0
scoreboard players add #total_californium_battery crazy_adventure.data 0
scoreboard players add #total_uranium_battery crazy_adventure.data 0
scoreboard players add #total_incredible_nuclear_reactor crazy_adventure.data 0
scoreboard players add #total_amazing_nuclear_reactor crazy_adventure.data 0
scoreboard players add #total_better_nuclear_reactor crazy_adventure.data 0
scoreboard players add #total_nuclear_reactor crazy_adventure.data 0
scoreboard players add #total_boss_altar crazy_adventure.data 0
scoreboard players add #total_californium_waste_barrel crazy_adventure.data 0
scoreboard players add #total_uranium_waste_barrel crazy_adventure.data 0
scoreboard players add #total_californium_ore crazy_adventure.data 0
scoreboard players add #total_californium_block crazy_adventure.data 0
scoreboard players add #total_deepslate_uranium_ore crazy_adventure.data 0
scoreboard players add #total_uranium_ore crazy_adventure.data 0
scoreboard players add #total_uranium_block crazy_adventure.data 0
tellraw @s [{"translate": "crazy_adventure.total_uranium_block","color":"gold"},{"score":{"name":"#total_uranium_block","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_uranium_ore","color":"gold"},{"score":{"name":"#total_uranium_ore","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_deepslate_uranium_ore","color":"gold"},{"score":{"name":"#total_deepslate_uranium_ore","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_californium_block","color":"gold"},{"score":{"name":"#total_californium_block","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_californium_ore","color":"gold"},{"score":{"name":"#total_californium_ore","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_uranium_waste_barrel","color":"gold"},{"score":{"name":"#total_uranium_waste_barrel","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_californium_waste_barrel","color":"gold"},{"score":{"name":"#total_californium_waste_barrel","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_boss_altar","color":"gold"},{"score":{"name":"#total_boss_altar","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_nuclear_reactor","color":"gold"},{"score":{"name":"#total_nuclear_reactor","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_better_nuclear_reactor","color":"gold"},{"score":{"name":"#total_better_nuclear_reactor","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_amazing_nuclear_reactor","color":"gold"},{"score":{"name":"#total_amazing_nuclear_reactor","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_incredible_nuclear_reactor","color":"gold"},{"score":{"name":"#total_incredible_nuclear_reactor","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_uranium_battery","color":"gold"},{"score":{"name":"#total_uranium_battery","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_californium_battery","color":"gold"},{"score":{"name":"#total_californium_battery","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_ore_extractor_1","color":"gold"},{"score":{"name":"#total_ore_extractor_1","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_ore_extractor_2","color":"gold"},{"score":{"name":"#total_ore_extractor_2","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_ore_extractor_3","color":"gold"},{"score":{"name":"#total_ore_extractor_3","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_ore_extractor_4","color":"gold"},{"score":{"name":"#total_ore_extractor_4","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_uranium_upgrader","color":"gold"},{"score":{"name":"#total_uranium_upgrader","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_oxidizer","color":"gold"},{"score":{"name":"#total_oxidizer","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_storm_stick_charger","color":"gold"},{"score":{"name":"#total_storm_stick_charger","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_fireball_stick_charger","color":"gold"},{"score":{"name":"#total_fireball_stick_charger","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_shield_stick_charger","color":"gold"},{"score":{"name":"#total_shield_stick_charger","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_fertilizer_stick_charger","color":"gold"},{"score":{"name":"#total_fertilizer_stick_charger","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_banana_sapling","color":"gold"},{"score":{"name":"#total_banana_sapling","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.total_banana_plant","color":"gold"},{"score":{"name":"#total_banana_plant","objective":"crazy_adventure.data"},"color":"yellow"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_barrel","color":"gray"},{"score":{"name":"#total_vanilla_barrel","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_cobblestone","color":"gray"},{"score":{"name":"#total_vanilla_cobblestone","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_emerald_block","color":"gray"},{"score":{"name":"#total_vanilla_emerald_block","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_end_rod","color":"gray"},{"score":{"name":"#total_vanilla_end_rod","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_furnace","color":"gray"},{"score":{"name":"#total_vanilla_furnace","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_gold_block","color":"gray"},{"score":{"name":"#total_vanilla_gold_block","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_iron_block","color":"gray"},{"score":{"name":"#total_vanilla_iron_block","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_jungle_sapling","color":"gray"},{"score":{"name":"#total_vanilla_jungle_sapling","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_obsidian","color":"gray"},{"score":{"name":"#total_vanilla_obsidian","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.vanilla_minecraft_polished_deepslate","color":"gray"},{"score":{"name":"#total_vanilla_polished_deepslate","objective":"crazy_adventure.data"},"color":"white"}]
tellraw @s [{"translate": "crazy_adventure.total_custom_blocks","color":"dark_aqua"},{"score":{"name":"#total_custom_blocks","objective":"crazy_adventure.data"},"color":"aqua"}]

