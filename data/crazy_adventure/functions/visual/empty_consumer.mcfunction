...
execute if score @s energy.storage matches 50.. run function crazy_adventure:work/empty_consumer
...
#Update model depends on BurnTime (on/off)
	execute if data block ~ ~ ~ {BurnTime:0s} run data modify entity @s Item.tag.CustomModelData set value 4220400
	execute unless data block ~ ~ ~ {BurnTime:0s} run data modify entity @s Item.tag.CustomModelData set value 4220401
